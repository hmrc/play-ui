...
trait Validators {
  ...
  def validateMandatoryPhoneNumber = { s: String =>
    s.matches("\\+?\\d+")
  }

  def validateOptionalPhoneNumber = { s: String =>
    s.matches("\\+?\\d*")
  }

  def validateSaUtr = { s: String =>
    s.matches("\\d{10}")
  }
...
object characterValidator {
  ...
  def containsValidPostCodeCharacters(value: String): Boolean =
    containsValidCharacters(value, invalidPostCodeCharacterRegex)
